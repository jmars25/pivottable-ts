  ...
  callWithJQuery(function($) {
    ...
    PivotData = (function() {
      function PivotData(input, opts) {
        ...
        this.sorted = false;
        if (!opts.lazy) {
          if (input.columnFormat && $.isEmptyObject(this.derivedAttributes) && !opts.filter) {
    ...
    PivotStream = (function() {
      function PivotStream(opts) {
        var pdOpts, ref, ref1, ref2;
        if (opts == null) {
          opts = {};
        }
        this.progressInterval = (ref = opts.progressInterval) != null ? ref : 2000;
        this.onProgress = (ref1 = opts.onProgress) != null ? ref1 : function() {};
        this.onComplete = (ref2 = opts.onComplete) != null ? ref2 : function() {};
        this._count = 0;
        pdOpts = $.extend({}, opts, {
          lazy: true
        });
        this.pivotData = new PivotData({}, pdOpts);
      }

      PivotStream.prototype.push = function(record) {
        this.pivotData.pushRecord(record);
        this._count++;
        if (this._count % this.progressInterval === 0) {
          return this.onProgress(this.pivotData, this._count);
        }
      };

      PivotStream.prototype.done = function() {
        return this.onComplete(this.pivotData, this._count);
      };
      ...
      PivotStream.prototype.fromFetch = function(url, fetchOpts) {
        ...
        return fetch(url, fetchOpts != null ? fetchOpts : {}).then(function(res) {
          var buffer, decoder, pump, reader;
          if (!res.ok) {
          ...
          pump = function() {
            return reader.read().then(function(arg) {
              var done, l, len1, line, lines, value;
              done = arg.done, value = arg.value;
              if (done) {
                if (buffer.trim().length > 0) {
                  try {
                    self.push(JSON.parse(buffer));
                  } catch (error) {}
                }
                self.done();
                return;
